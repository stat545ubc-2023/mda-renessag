...
library(rstatix) #includes function to identify outliers
library(viridis) #remove
```
...
1. Which physical attribute (variable) is the strongest indicator of diagnosis? 
2. Is there a numerical range of this attribute (variable) that can define a benign vs. malignant tumor? 
3. Do samples with large margins of error (SE value) still align with the above ranges of benign
vs. malignant?
4. For discrete ranges of this attribute (variable), what is the likelihood that the sample is benign vs. malignant?
<!----------------------------------------------------------------------------->
...
# `cancer_sample` Data Set Analysis
...
#### Purpose  
Group by B/M, re-calculate the means, normalize.

```{r}
fold_diff <- cancer_sample %>%
  group_by(diagnosis) %>%
  summarize_all(mean) %>%
  summarize(diagnosis,
...
```{r}
concavity_box <- ggplot(data = cancer_sample,
                        aes(x = diagnosis,
                            y = concavity_mean)) +
...
concave_points_box <- ggplot(data = cancer_sample,
                             aes(x = diagnosis,
...
area_box <- ggplot(data = cancer_sample,
                   aes(x = diagnosis,
                       y = area_mean)) +
  geom_boxplot(aes(fill = factor(diagnosis, c("M", "B"))),
               alpha = 0.75,
...
ggarrange(concavity_box, concave_points_box, area_box,
...
#### Conclusion  
Concave points is the best marker.
...
```{r}
concave_points_analysis <- cancer_sample %>%
...
print(concave_points_analysis)
```

```{r}
ggplot(data = concave_points_analysis,
...
```{r}
outlier_analysis_concave_points <- cancer_sample %>%
  select(ID, diagnosis, concave_points_mean) %>%
  group_by(diagnosis) %>%
  identify_outliers(concave_points_mean)
print(outliers_concave_points)
```

```{r}
concave_points_w_outliers <- outliers_concave_points %>%
  right_join(cancer_sample, by = c("ID", "diagnosis", "concave_points_mean")) %>%
  select(c(diagnosis:is.outlier), concave_points_se)
print(concave_points_w_outliers)
```


```{r}
ggplot(data = concave_points_w_outliers,
...
```{r}
concave_points_analysis %>%
  group_by(concave_points_level) %>%
  summarize("B count" = length(which(diagnosis == "B")),
            "M count" = length(which(diagnosis == "M")),
            "%B" = length(which(diagnosis == "B"))/n(),
            "%M" = length(which(diagnosis == "M"))/n())
```

```{r}
concave_points_level_count <- ggplot(data = concave_points_analysis,
                                    aes(x = concave_points_level)) +
...
concave_points_level_prop <- ggplot(data = concave_points_analysis,
                                    aes(x = concave_points_level)) +
  geom_bar(aes(fill = factor(diagnosis, c("M", "B"))),
           position = "fill") +
  labs(x = "Concave Points Level",
...
ggarrange(concave_points_level_count, concave_points_level_prop,
          nrow = 1, ncol = 2,
          common.legend = TRUE, legend = "bottom")
```
